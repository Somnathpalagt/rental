...
export default function Auth() {
  ...
  async function handleAuth(e: React.FormEvent) {
    e.preventDefault();
    if (loginAttempts >= 5) return toast.error('Too many failed attempts, try again later.');
  ...
  return (
    <form onSubmit={handleAuth} className="space-y-4">
      ...
      <button type="button" onClick={() => setIsSignUp(!isSignUp)} className="text-blue-500 w-full">
